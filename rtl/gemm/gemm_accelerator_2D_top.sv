...
module gemm_accelerator_2D_top #(
  ...
  input  logic signed [ParforK-1:0][  InDataWidth-1:0] sram_a_rdata_i,
  input  logic signed [ParforN-1:0][ParforK-1:0][  InDataWidth-1:0] sram_b_rdata_i,
  output logic signed [ParforN-1:0][ OutDataWidth-1:0] sram_c_wdata_o,
  ...
  assign K_packed_depth = (K_size_i + ParforK - 1) / ParforK;
  logic [AddrWidth-1:0] N_packed_depth;
  assign N_packed_depth = (N_size_i + ParforN - 1) / ParforN;
  ...
  // The MAC PE instantiation and data path logics
   ...
   genvar  n;
    ...
    for (n = 0; n < ParforN; n++) begin : gem_mac_pe_n
    mac_2D #(
