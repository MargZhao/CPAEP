...
module gemm_controller_2D #(
  parameter int unsigned AddrWidth = 16,
  parameter int unsigned NumK = 16,
  parameter int unsigned NumN = 4
)(
  input  logic clk_i,
  input  logic rst_ni,
  input  logic start_i,
  ...
  assign K_packed_depth = (K_size_i + NumK - 1) / NumK;
  logic [AddrWidth-1:0] N_packed_depth;
  assign N_packed_depth = (N_size_i + NumN - 1) / NumN;
  ...
  //          parfor ni = 0 to X-1
  //          parfor ki = 0 to Y-1
  //            C[m][n*X + ni] += A[m][k*Y + ki] * B[k*Y + ki][n*X + ni]
  //TODO1:
  // for m = 0 to M-1
  //   for n = 0 to N-1/4
  //     for k = 0 to K-1/16
  //          parfor ni = 0 to 3
  //          parfor ki = 0 to 15
  //            C[m][n*4 + ni] += A[m][k*16 + ki] * B[16*Y + ki][n*4 + ni]
  //       
  //
  //TODO2:
  // for m = 0 to M-1/4
  ...
  //          parfor ni = 0 to 3
  //          parfor ki = 0 to 3
  //            C[m*4 + mi][n*4 + ni] += A[m*4 + mi][k*16 + ki] * B[16*Y + ki][n*4 + ni]
  //       
  //
